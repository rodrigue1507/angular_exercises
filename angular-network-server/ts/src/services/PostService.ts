...
import { Channel, Post, UserCommentEdge, LikeEdge, PostCommentEdge, User } from "../database/graph";
...
export class PostService {
    ...
    find(id: string): Promise<Post> {
        return this.db.first<any>(`match (u:User)-[]-(p:Post {id: {id}})-[:CHANNEL_POST]-(c:Channel) return p, u, c`, { id })
            .then(r => {
                if (r) {
                    const post: any = new Post(r.p);
                    post.user = new User(r.u);
